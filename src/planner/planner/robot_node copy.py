...
class Robot(Node):
    ...
    def start_server(self):
        ...
        self.robot_server = self.create_service(
            SendPlan, 'robot_%s_server' % (self.id), self.receive_message
        )
        self.robot_communication_sync_server = self.create_service(
            Action, 'robot_%s_communication_sync_server' % (
                self.id), self.receive_sync_message
        )
    ...
    def notifyOnline(self):
        ...
        self.action_request.action = ','.join(('online', 'robot', self.id))
    ...
    def notifySuccess(self):
        self.action_request = Action.Request()
        self.action_request.action = ','.join(('action_complete',))
        return self.coordinator_communication_sync_client.call(
            self.action_request
        )
